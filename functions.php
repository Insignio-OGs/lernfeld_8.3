...
<?php
    ...
    */
    ...
    /**
    * Prüft ob in der derzeitigen Session der angegebene Key gesetzt ist
    * Parameter:
    *  - $key = Key nach dem im Session Array gesucht wird
    * @param string $key
    */
    function check_session($key) {
        if(!isset($_SESSION[$key])) {
            echo "<script>window.location.href = 'error.htm'</script>";
        }
    }

    /**
    * Prüft ob der angemeldete User ein Admin oder normaler benuzer ist und führt zum abbruch des
    * Programms fals der Benutzer nicht die Berechtigungen hat um die seite zu sehen
    * Parameter:
    *  - $userlvl_needed    = Benutzer Level das benötigt wird um die Seite zu sehen
    *  - $index_location   = Pfad zur php Datei auf die verwiesen wird wenn der Wert in der Session nicht existiert
    * @param int $userlvl_needed
    * @param string $index_location
    */
    function check_user($userlvl_needed, $index_location) {
        if ($_SESSION["userlvl"] < $userlvl_needed) {
            exit("<div class='container'>
                    <p class='error'>
                        <h2>Zugriff verweigert!</h2>
                        <p><a href='" . $index_location . "'>Zurück zum Login</a></p>
                    </p>
                </div>");
        }
    }
    ...
    /**
    * Öffnet die angegeben Datei und liest den Datenbank Namen aus der ersten Zeile der Datei.
    * Stellt die Verbindung zu DB her und gib sie zurück ($con)
    * Parameter:
    *  - $file = Pfad mit Namen der Datei (ausgehend von der functions.inc.php)
    * @param string $file
    * @return mixed 
    */
    function get_dbConnection($file) {
        $fp = @fopen($file, r);
        if(!$fp) {
            exit("Datei kann zum lesen nicht geöffnet werden nicht geöffnet werden");
        }
        $dbName = fgets($fp);
        fclose($fp);
        $con = mysqli_connect("", "root", "", $dbName) or die("verbindung fehlgeschlagen");
        return $con;
    }

    /**
    * Öffnet die angegeben Datei und liest den Datenbank Namen aus der ersten Zeile der Datei.
    * Gib den Namen der DB zurück
    * Parameter:
    *  - $file = Pfad mit Namen der Datei (ausgehend von der functions.inc.php)
    * @param string $file
    * @return string 
    */
    function get_dbName($file) {
        $fp = @fopen($file, r);
        if(!$fp) {
            exit("Datei kann zum lesen nicht geöffnet werden nicht geöffnet werden");
        }
